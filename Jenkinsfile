...
pipeline {
  ...
  stages {
    stage('SonarQube Analysis') {
      steps {
        withSonarQubeEnv('Eniworks') {
          container('sonarscan') {
              sh "sonar-scanner -Dsonar.projectKey=D-DASH -Dsonar.sources=app,test"
          }
        }
      }
    }
    stage('Build/Push Docker Image') {
      steps {
        container('kaniko') {
            sh "/kaniko/executor --context . --cache --destination=enigodupont/d-dashboard:default_tag_change_me --destination=enigodupont/d-dashboard:$BUILD_NUMBER"
        }
      }
    }
        ...
        withCredentials(bindings: [sshUserPrivateKey(credentialsId: '8fe21223-8d56-49ea-8eb5-6dc0171c102f', keyFileVariable: 'GIT_SSH_KEY', passphraseVariable: '', usernameVariable: '')]) {
          container('git') {
            ...
            sh '''
              ...
              git clone git@github.com:enigodupont/D-Dashboard.git
              cd D-Dashboard
